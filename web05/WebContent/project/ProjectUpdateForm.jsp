...
<li>
	<label for="state">상태${project.state }</label>
	<select id="state" name="state">
		<option value="0" ${projcet.state==0? "selected" : ""}>준비</option>
		<option value="1" ${projcet.state==1? "selected" : ""}>진행</option>
		<option value="2" ${projcet.state==2? "selected" : ""}>완료</option>
		<option value="3" ${projcet.state==3? "selected" : ""}>취소</option>
	</select>
</li>
<li>
...
<input type="button" value="리스트로 돌아가기" onclick="location.href='list.do'">
</form>
